...
export default function Page() {
    ...
    <PCourse className={"my-6"}>Il est important de bien comprendre la différence entre les <SpanCourse>Params</SpanCourse> et les <SpanCourse>SearchParams</SpanCourse> d&apos;une url.</PCourse>
    ...
    <PCourse className={'my-6'}>C&apos;est un <SpanCourse>Params</SpanCourse>.</PCourse>


    <Link href="/next-js/params.jpg" target="_blank"><Image className={'shadow-md rounded-3xl'} src={imgParams} alt="Params" /></Link>


    <PCourse className={'my-6'}>Ici <SpanCourse>category</SpanCourse> et <SpanCourse>slug</SpanCourse> sont des paramètres dynamiques que nous verrons comment récupérer grâce à la propriété <SpanCourse>Params</SpanCourse>.</PCourse>
    ...
    <PCourse className={'my-6'}>Ne pas confondre avec les <SpanCourse>SearchParams</SpanCourse> qui sont les paramètres qui viennent apres le <SpanCourse>?</SpanCourse> d&apos;une url et sont séparés par un <SpanCourse>&</SpanCourse>.</PCourse>
    ...
    <PCourse className={"my-6"}>Il est important de bien comprendre la différence entre les deux car ils sont utilisés de manière différente.</PCourse>
    ...
    <PCourse className={'my-6'}>Les <SpanCourse>Params</SpanCourse> sont injectés dans les <SpanCourse>Layout</SpanCourse> et <SpanCourse>Pages</SpanCourse> des <SpanCourse>Server Components</SpanCourse> tandis que les <SpanCourse>SearchParams</SpanCourse> UNIQUEMENT dans les <SpanCourse>Pages</SpanCourse>.</PCourse>
    ...
    <PCourse className={"my-6"}>Exemple de récupération d&apos;un <SpanCourse>Params</SpanCourse> en <SpanCourse>Typescript</SpanCourse> avec un petit aperçu d&apos;une requête <SpanCourse>Prisma</SpanCourse> :</PCourse>
    ...
    <PCourse className={"my-6"}>Le <SpanCourse>type</SpanCourse> des <SpanCourse>SearchParams</SpanCourse> est un peu différent, car il peut être un tableau :</PCourse>
    <MultiLignesCodeWrapper className={"mb-6"}>
        <CodeCourse langage={"tsx"}>
    ...
    <PCourse>Dans un <SpanCourse>Client Component</SpanCourse>, il faudra récupérer les <SpanCourse>Params</SpanCourse> et <SpanCourse>SearchParams</SpanCourse> à l&apos;aide des hooks <SpanCourse>useParams</SpanCourse> et <SpanCourse>useSearchParams</SpanCourse>.</PCourse>
    ...
    <PCourse className={"my-6"}>Contrairement au <SpanCourse>Client Side Rendering</SpanCourse> (CSR), où le navigateur du client exécute le JavaScript pour générer le contenu, avec SSR, le serveur produit une page HTML complète qui est immédiatement visible par l&apos;utilisateur dès qu&apos;elle est chargée.</PCourse>

    <PCourse className={"mb-2"}>Voici comment ça fonctionne :</PCourse>
    <UlCourse>
    ...
    <PCourse className={"my-6"}>En réalité les <SpanCourse>Client Component</SpanCourse> sont aussi rendus une premiere fois <SpanCourse>Côté Serveur</SpanCourse>.</PCourse>
    ...
    <PCourse className={"mt-6"}>Pour obtenir une liste complète, <Link className={cn(buttonVariants({
        variant: "link"
    }), "p-0 text-2xl")} href="https://nextjs.org/docs/getting-started/installation" target="_blank">rendez-vous sur le site officiel de Next.js</Link>.</PCourse>
    ...
    <PCourse className={"my-6"}>Il récupère la propriété <SpanCourse>children</SpanCourse> pour encapsuler le contenu de la page et peut ajouter des éléments comme un menu ou un pied de page. Il peut aussi récupérer les <SpanCourse>Params</SpanCourse>, mais pas les <SpanCourse>SearchParams</SpanCourse>.</PCourse>
    ...
    <PCourse className={'my-6'}>Ce composant peut contenir du code JSX, des composants React, et toute la logique nécessaire pour afficher la page. Il est au cœur de la structure des routes dans Next.js, permettant de créer des pages dynamiques et interactives.</PCourse>
    ...
    <PCourse className={'my-6'}>Dans l&apos;idéal, on le gardera en tant que <SpanCourse>Server Component</SpanCourse> afin de récupérer les données depuis la base de données pour afficher la page plus rapidement et éviter les erreurs d&apos;hydratation.</PCourse>
    ...
    <PCourse className={'my-6'}>Vous pouvez avoir plusieurs routes cela fonctionne comme les pages, mais elles renvoient en général du <SpanCourse>JSON</SpanCourse>.</PCourse>
    ...
    <PCourse className={'my-6'}>Cela est très pratique pour sécuriser une route en fonction de l&apos;état d&apos;authentification de l&apos;utilisateur.</PCourse>
    ...
    <PCourse className={"my-6"}>Dans l&apos;exemple ci-dessous le matcher correspond aux routes que doit intercepter le middleware.</PCourse>
    ...
    <PCourse className={"my-6"}>Pour les métadonnées, <SpanCourse>Next.js</SpanCourse> propose un exemple dans le layout. Vous pouvez exporter une constante <SpanCourse>metadata</SpanCourse> pour personnaliser les méta-datas de chaque page.</PCourse>
    ...
    <PCourse className={"my-6"}>Elle est très utile pour la navigation et le routage, car elle permet de charger les pages de manière asynchrone, sans recharger toute la page.</PCourse>

    <PCourse>Cela améliore la performance et l&apos;expérience utilisateur, en évitant les temps de chargement inutiles.</PCourse>

    <PCourse className={'my-6'}>Voici un exemple d&apos;utilisation de la balise <SpanCourse>Link</SpanCourse> :</PCourse>
